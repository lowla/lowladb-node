(function(exports) {
  ...
  var LowlaId = require('./datastore/lowlaId').LowlaId;
  ...
  var LowlaAdapter = function(options) {
    ...
    if (!config.datastore) {
      ...
      adapter.logger.debug("Adapter is waiting for Datastore...");
          ...
          promises.push(
            adapter.config.datastore.getDocument(lowlaId)
              .then(function (doc) {
                ...
                throw new Error("Document retrieval failed with error for " + lowlaId.getClientId());
              })
          );
