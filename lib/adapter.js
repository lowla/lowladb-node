(function(exports) {
  ...
  LowlaAdapter.prototype.configureRoutes = function(app) {
    ...
    app.get('/_lowla/pull', pullFunc);
    app.post('/_lowla/pull', pullFunc);

    app.post('/_lowla/push', function(req, res, next) {
  ...
  LowlaAdapter.prototype._openCollection = function(clientId) {
    ...
    if (-1 != dollar) {
      ...
      clientNs = clientId.substring(0, dollar);
    }
    var collectionName = clientNs.substring(dot + 1);
    var lowlaDocId = {
      dbName: dbName,
    ...
    return collectionHash[clientNs] = Q.Promise(function(resolve, reject) {
