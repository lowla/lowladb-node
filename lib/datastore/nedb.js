(function (exports) {
  ...
  LowlaNEDB.namespaceFromId = namespaceFromId;
  LowlaNEDB.idFromComponents = idFromComponents;
      ...
      return new _p.Promise(function (resolve, reject) {
        db.find({}, function (err, docs) {
          ...
          docs.forEach(function (doc) {
            docHandler(LowlaNEDB.idFromComponents(namespace, doc._id), doc._version, false, doc);
          });
  ...
  function updateDocumentByOperations(lowlaId, versionPreUpdate, updateOps) {
    ...
    return this._openNamespace(idObj)
      .then(function(db) {
        return new _p.Promise(function(resolve, reject) {
          db.update({_id: idObj.id}, updateOps, {upsert: true}, function(err, numRep, newDoc) {
            if (err) { return reject(err); }
            if (newDoc) {
