    ...
    return new _prom.Promise(function(resolve, reject){
      try {
        datastore.config.db.collection(lowlaId.collectionName, function (err, collection) {
          ...
          var isNew = false;
          if (!lowlaId.id) {
            reject(new Error('Datastore.updateDocumentByOperations: id must be specified'));
          }
  ...
  Datastore.prototype.removeDocument = function (lowlaId, versionPreDelete) {  //todo versions for conflicts
    ...
    datastore.config.db.collection(lowlaId.collectionName, function (err, collection) {
      ...
      collection.remove({_id: lowlaId.id}, function(err, numRemoved) {
        ...
        if(1>numRemoved) {
          return deferred.reject(new Error("Document was not removed but MongoDb did not return an error.  Already deleted?")); //todo
    ...
    datastore.config.db.collection(lowlaId.collectionName, function (err, collection) {
      ...
      collection.findOne({_id:lowlaId.id}, function (err, doc) {
        if (err) {
          ...
          deferred.reject({isDeleted:true});
        }
        ...
        var deferred = _prom.defer();
          datastore.findInCollection(collectionName, {}).then(function (cursor) {
            ...
            stream.on('data', function(doc) {
              ...
              docHandler.write(lowlaId, doc._version, false, doc);
              ++cnt;
            });
          });
